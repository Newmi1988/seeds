...
input <- approxfun(x = inputData$t, y = inputData$u, rule = 2, method = 'linear')
outN <- deSolve::ode(y = x0, times = times, func = modelJakStat, parms = parameters, input = input)


matplot(x = outN[,1], y = outN[,-1], type = 'l')


## trying the log transformation for jackstat
...
input <- approxfun(x = inputData$t, y = inputData$u, rule = 2, method = 'linear')
out <- deSolve::ode(y = x0, times = times, func = modelJakStat, parms = parameters, input = input)
