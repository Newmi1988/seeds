...
meas <- measure[,c(1,2,4)]


testModel <- odeModel(func = modelJakStat, parms = parameters, input = inputData, measFunc = measJakStat, y = x0, meas = meas)

# Funktion ohne return. Schreibt basierend auf dem odeModel-Object die c-Klasse
genCCode(odeModel = testModel, bden = TRUE, logTrans = c(1))
# Optionen
# bden : Erzeugt C-Datei welche die Formatierung für die Verwendung mit BDEN benutzt
# logTrans : Indezes geben an welche Komponenten des Zustandsvektors Transformiert werden sollen


# nominale Lösung des Modells betrachten (ist complett unabhängig von der Funktion genCCode)
out <- nominalSol(testModel, logTrans = c(1))
plot(out)
