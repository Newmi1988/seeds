...
This package is an implementation of the dynamic elastic net approach. There are two algorithms implemented that aim to calculate missing interactions between states of an ode, the explain and minimize a systematic error within the model-equation. The hidden inputs should be an hint as to which states are missing interactions.

Both algorithm make have use of the deSolve package and Rtools to compile the given models.

## The SGDN (sparse greedy dynamic net) Algorithm

The first Algorithm is a greedy method for generating a sparse solution for the hidden inputs. Its an iterative gradient descent method based on the costate equations of the system. Rtools is suggested, because of the immense computational cost of the step size of the gradient step.
...
- **times**: time steps on which the model should be evaluated
...
The class also provides a simple way to calculate and display the nominal solution of an created odeModel object with the print function.
...
The states of the model have to be written with the letter $x$ following by a number representing the state. If an input is needed for the model the input takes the letter u. This restrictions are made to ensure that the automatic differentiation will work.
...
The results are returned as list of resultsSeeds objects, which store the calculated states and measurements and the hidden inputs. A standard plot function is given to display the results. Functions for annotating the plots are also included in the package.
...
All values can be retrieved from the slots of the resultsSeeds object. For further information look into the documentation of the functions.
...
# Bayesian Dynamic Elastic Net
 ...
 Full Bayesian algortihm to detect hidden inputs in ODE based models.The algortihm is an extension of the Dynamic Elastic Net algorithm (Engelhardt et al. 2016) inspired by the Elastic-Net Regression.
...
The algorithm can be executed in a smiliar fashion as the sgdn algorithm. However computational time is much higher. See demostrations on how to use the algorithm.